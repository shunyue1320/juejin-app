<template>
  <div class="pin-container">
    ...
    <div class="pin-aside">
      <RecommendPin v-if="recommendPins.length" :list="recommendPins" />
...
export default {
  ...
  async asyncData({ app, store }) {
    ...
    console.log("pinCategoryList", pinCategoryList);
    if (!(pinCategoryList[pinCategoryList.length - 1].topic_id > 0)) {
      await app.$api.getRecommendTopics({
        ...
        if (res.err_no === 0) {
          ...
          store.commit('category/UPDATE_PIN_CATEGORY_LIST', list)
        }
      })
    }
    return {
